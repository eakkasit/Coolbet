 ...
 * @since      1.0.0
 *
 * @package    TOTOIT_Posts_Widget
 * @subpackage TOTOIT_Posts_Widget/includes
 */
...
/**
 * TOTOIT Posts Widget Class
 */
class TOTOIT_Posts_Widget extends WP_Widget {
	...
	public function __construct() {
		...
		parent::__construct(
			$this->widget_slug,
			__( 'TOTOIT Posts Widget', $this->widget_text_domain ),
			$widget_opts
		);
		...
		// Setup our get title/AJAX callback
		add_action( 'wp_ajax_dpe_fp_get_title', array( &$this, 'title_list' ) );

		// Setup our get games/AJAX callback
		add_action( 'wp_ajax_dpe_fp_get_games', array( &$this, 'games_list' ) );
		
	}


	/** get post ids from all widget */
	public function getIDs(){
		$options = get_option( 'widget_' . $this->widget_slug  );
		$notin = array();
		if( $options ) {
			...
			foreach( $options as $key => $value ) {
									...
									// If we've got pids to convert...
									if( isset( $value['pids'] ) && ! empty( $value['pids'] ) ) {

										$notin = array_merge($notin,$value['pids']);
	...
	/** get post ids from exclude story */
	public function getExcludeIDs(){
		...
		if(!empty($exclude_query->posts)){
			// echo '<pre>';
			// print_r($exclude_query->posts);
			// echo '</pre>';
			foreach($exclude_query->posts as $item){
				//echo $item->ID.'<br>';
				$exclude = array_merge($exclude,array($item->ID));
				// echo '<pre>';
				// print_r($exclude);
				// echo '</pre>';
			}
	...
	public function widget( $args, $instance ) {
		...
		$query_args['ignore_same_posts']  = $notin;
		...
		$totoit_posts = new WP_Query( $query_args );
	...
	public function update( $new_instance, $old_instance ) {
		...
		$instance['notin']		= ( isset(  $new_instance['notin'] ) ? (int) $new_instance['notin'] : '0' );
	 ...
	 * Will look in the totoit-posts-widget/ directory in a theme
	...
	/**
	 * Return a list of title for the chosen posts
	 */
	public function title_list( $title, $sel_terms = array() ) {

		if ( empty( $title ) && isset( $_GET['title'] ) )
			$title = esc_attr( $_GET['title'] );
		
		if ( empty( $title ) || 'none' == $title ) {
			echo false;
			if( isset ( $_GET['action'] )  && 'dpe_fp_get_title' === $_GET['action'] ) {
		...
		$title = $wpdb->esc_like(stripslashes($_GET['title'])).'%'; //escape for use in LIKE statement
		...
		foreach( $results as $r ){
			...
			$titles[$i]['value'] = addslashes($r->ID);
			$i++;
		}
	...
	public function setup_defaults() {
		...
		$this->orderbys = array(
			...
			'popular'		=> __( 'Popular', $this->widget_text_domain ),
		);
