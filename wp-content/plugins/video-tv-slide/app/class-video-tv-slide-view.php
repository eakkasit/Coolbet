...
require_once 'class-video-tv-slide-list-table.php';
require_once 'class-video-tv-slide-creator.php';

class Video_TV_Slide_View {
	...
	function add_metaboxes() {
		add_meta_box('overview_features', __('WonderPlugin Gallery Features', 'video_tv_slide'), array($this, 'show_features'), 'video_tv_slide_overview', 'features', '');
		add_meta_box('overview_upgrade', __('Upgrade to Commercial Version', 'video_tv_slide'), array($this, 'show_upgrade_to_commercial'), 'video_tv_slide_overview', 'upgrade', '');
		add_meta_box('overview_news', __('WonderPlugin News', 'video_tv_slide'), array($this, 'show_news'), 'video_tv_slide_overview', 'news', '');
		add_meta_box('overview_contact', __('Contact Us', 'video_tv_slide'), array($this, 'show_contact'), 'video_tv_slide_overview', 'contact', '');
	}
		        ...
		        foreach ( $rss_items as $item )
		        {
		        	...
		        	<li>
		                <a href="<?php echo esc_url( $item->get_permalink() ); ?>" target="_blank" 
		                    title="<?php printf( __( 'Posted %s', 'video_tv_slide' ), $item->get_date('j F Y | g:i a') ); ?>">
	...
	function print_overview() {
		...
		?>
		<div class="wrap">
		<div id="icon-video-tv-slide" class="icon32"><br /></div>
			
		<h2><?php echo __( 'WonderPlugin Gallery', 'video_tv_slide' ) . ( (WONDERPLUGIN_GALLERY_VERSION_TYPE == "C") ? " Commercial Version" : " Free Version") . " " . WONDERPLUGIN_GALLERY_VERSION; ?> </h2>
			...
			<div class="welcome-panel-content">
				...
				<div class="welcome-panel-column-container">
					<div class="welcome-panel-column">
						<h4>Get Started</h4>
						<a class="button button-primary button-hero" href="<?php echo admin_url('admin.php?page=video_tv_slide_add_new'); ?>">Create A New Gallery</a>
					</div>
					<div class="welcome-panel-column">
						<h4>More Actions</h4>
						<ul>
							<li><a href="<?php echo admin_url('admin.php?page=video_tv_slide_show_items'); ?>" class="welcome-icon welcome-widgets-menus">Manage Existing Galleries</a></li>
		...
		<div id="dashboard-widgets-wrap">
			<div id="dashboard-widgets" class="metabox-holder columns-2">
	                 ...
	                 <div class="postbox-container">
	                    <?php 
	                    do_meta_boxes( 'video_tv_slide_overview', 'features', '' ); 
	                    do_meta_boxes( 'video_tv_slide_overview', 'contact', '' ); 
	                    ?>
	                </div>
	                ...
	                <div class="postbox-container">
	                    ...
	                    if (WONDERPLUGIN_GALLERY_VERSION_TYPE != "C")
	                    	do_meta_boxes( 'video_tv_slide_overview', 'upgrade', ''); 
	                    do_meta_boxes( 'video_tv_slide_overview', 'news', ''); 
	                    ?>
	                </div>
	...
	function print_edit_settings() {
		?>
				...
				<h2><?php _e( 'Settings', 'video_tv_slide' ); ?></h2>
				<?php

				if ( isset($_POST['save-gallery-options']) && check_admin_referer('video-tv-slide', 'video-tv-slide-settings') )
				{		
		        ...
		        <?php wp_nonce_field('video-tv-slide', 'video-tv-slide-settings'); ?>
	...
	function print_register() {
		...
		<h2><?php _e( 'Register', 'video_tv_slide' ); ?></h2>
		<?php
				
		if (isset($_POST['save-gallery-license']) && check_admin_referer('video-tv-slide', 'video-tv-slide-register') )
		{		
		...
		else if (isset($_POST['deregister-gallery-license']) && check_admin_referer('video-tv-slide', 'video-tv-slide-register') )
		{	
		...
		?> <div style="padding-left:10px;padding-top:12px;"> <?php
			if (empty($key)) { ?>
				<form method="post">
				<?php wp_nonce_field('video-tv-slide', 'video-tv-slide-register'); ?>
				<table class="form-table">
				<tr>
					<th>Enter Your License Key:</th>
					<td><input name="video-tv-slide-key" type="text" id="video-tv-slide-key" value="" class="regular-text" /> <input type="submit" name="save-gallery-license" id="save-gallery-license" class="button button-primary" value="Register License Key"  />
					</td>
				</tr>
				</table>
				</form>
			<?php } else { ?>
				<form method="post">
				<?php wp_nonce_field('video-tv-slide', 'video-tv-slide-register'); ?>
				<p>You have entered your license key and this domain has been successfully registered. &nbsp;&nbsp;<input name="video-tv-slide-key" type="hidden" id="video-tv-slide-key" value="<?php echo esc_html($key); ?>" class="regular-text" /><input type="submit" name="deregister-gallery-license" id="deregister-gallery-license" class="button button-primary" value="Deregister Your License Key"  /></p>
				</form>
	...
	function print_items() {
		...
		<h2><?php _e( 'Manage Galleries', 'video_tv_slide' ); ?> <a href="<?php echo admin_url('admin.php?page=video_tv_slide_add_new'); ?>" class="add-new-h2"> <?php _e( 'New Gallery', 'video_tv_slide' ); ?></a> </h2>
		...
		if ( !is_object($this->list_table) )
			$this->list_table = new Video_TV_Slide_List_Table($this);
	...
	function print_item()
	{
		...
		<h2><?php _e( 'View Gallery', 'video_tv_slide' ); ?> <a href="<?php echo admin_url('admin.php?page=video_tv_slide_edit_item') . '&itemid=' . $_REQUEST['itemid']; ?>" class="add-new-h2"> <?php _e( 'Edit Gallery', 'video_tv_slide' ); ?>  </a> </h2>
		
		<div class="updated"><p style="text-align:center;">  <?php _e( 'To embed the gallery into your page, use shortcode: ', 'video_tv_slide' ); ?> <?php echo esc_attr('[video_tv_slide id="' . $_REQUEST['itemid'] . '"]'); ?></p></div>

		<div class="updated"><p style="text-align:center;">  <?php _e( 'To embed the gallery into your template, use php code: ', 'video_tv_slide' ); ?> <?php echo esc_attr('<?php echo do_shortcode(\'[video_tv_slide id="' . $_REQUEST['itemid'] . '"]\'); ?>'); ?></p></div>
	...
	function print_add_new() {
		...
		if ( !empty($_POST['video-tv-slide-save-item-post-value']) && !empty($_POST['video-tv-slide-save-item-post']) && check_admin_referer('video-tv-slide', 'video-tv-slide-saveform') )
		{
			$this->save_item_post($_POST['video-tv-slide-save-item-post-value']);
			return;
		}
		...
		<h2><?php _e( 'New Gallery', 'video_tv_slide' ); ?> <a href="<?php echo admin_url('admin.php?page=video_tv_slide_show_items'); ?>" class="add-new-h2"> <?php _e( 'Manage Galleries', 'video_tv_slide' ); ?>  </a> </h2>
		
		<?php 
		$this->creator = new Video_TV_Slide_Creator($this);		
	...
	function print_edit_item()
	{
		...
		<h2><?php _e( 'Edit Gallery', 'video_tv_slide' ); ?> <a href="<?php echo admin_url('admin.php?page=video_tv_slide_show_item') . '&itemid=' . $_REQUEST['itemid']; ?>" class="add-new-h2"> <?php _e( 'View Gallery', 'video_tv_slide' ); ?>  </a> </h2>
	...
	function save_item_post($item_post) {
		...
		if (isset($ret['success']) && $ret['success'] && isset($ret['id']) && $ret['id'] >= 0) 
		{
			...
			echo "<a href='" . admin_url('admin.php?page=video_tv_slide_edit_item') . '&itemid=' . $ret['id'] . "' class='add-new-h2'>Edit Gallery</a>";
			echo "<a href='" . admin_url('admin.php?page=video_tv_slide_show_item') . '&itemid=' . $ret['id'] . "' class='add-new-h2'>View Gallery</a>";
			echo "</h2>";
					
			echo "<div class='updated'><p>The gallery has been saved and published.</p></div>";
			echo "<div class='updated'><p>To embed the gallery into your page or post, use shortcode:  [video_tv_slide id=\"" . $ret['id'] . "\"]</p></div>";
			echo "<div class='updated'><p>To embed the gallery into your template, use php code:  &lt;?php echo do_shortcode('[video_tv_slide id=\"" . $ret['id'] . "\"]'); ?&gt;</p></div>"; 
		}
		else
		{
	...
	function import_export()
	{
		?>
		<div class="wrap">
			<div id="icon-video-tv-slide" class="icon32">
				<br />
			</div>

			<h2>
				<?php _e( 'Import/Export', 'video_tv_slide' ); ?>
			</h2>
			...
			<ul class="wonderplugin-tab-buttons-horizontal"
				...
				<li
					...
					<?php _e( 'Import', 'video_tv_slide' ); ?></li>
				<li class="wonderplugin-tab-button-horizontal"><span
					class="dashicons dashicons-upload" style="margin-right: 8px;"></span>
					<?php _e( 'Export', 'video_tv_slide' ); ?></li>
					
				<li class="wonderplugin-tab-button-horizontal"><span class="dashicons dashicons-search" style="margin-right:8px;"></span><?php _e( 'Search and Replace', 'video_tv_slide' ); ?></li>
			</ul>
			...
			<ul class="wonderplugin-tabs-horizontal"
				...
				<li
					...
					<?php 
					if (isset($_POST['wp-import']) && isset($_FILES['importxml']) && check_admin_referer('video-tv-slide', 'video-tv-slide-import'))
						$import_return = $this->controller->import_gallery($_POST, $_FILES);
					?>

					<form method="post" enctype="multipart/form-data">
						<?php wp_nonce_field('video-tv-slide', 'video-tv-slide-import'); ?>
						<?php 
				...
				if (empty($data)) {
					...
					<form method="post"
						action="<?php echo admin_url('admin-post.php?action=video_tv_slide_export'); ?>">
						<?php wp_nonce_field('video-tv-slide', 'video-tv-slide-export'); ?>
				...
				<form method="post">
	        	<?php wp_nonce_field('video-tv-slide', 'video-tv-slide-search-replace'); ?>
	        	<?php
	        	if (isset($_POST['wp-search-replace-submit']) && check_admin_referer('video-tv-slide', 'video-tv-slide-search-replace'))
