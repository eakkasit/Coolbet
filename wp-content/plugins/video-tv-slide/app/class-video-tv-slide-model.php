...
require_once 'video-tv-slide-functions.php';

class Video_TV_Slide_Model {
	...
	function get_upload_path() {
		...
		return $uploads['basedir'] . '/video-tv-slide/';
	}
	
	function get_upload_url() {
	
		$uploads = wp_upload_dir();
		return $uploads['baseurl'] . '/video-tv-slide/';
	}
	...
	function import_gallery($post, $files)
	{
		...
		add_filter( 'wp_check_filetype_and_ext', 'video_tv_slide_wp_check_filetype_and_ext', 10, 4);
		...
		remove_filter( 'wp_check_filetype_and_ext', 'video_tv_slide_wp_check_filetype_and_ext');
	...
	{
		if ( !check_admin_referer('video-tv-slide', 'video-tv-slide-export') || !isset($_POST['allgallery']) || !isset($_POST['galleryid']) || !is_numeric($_POST['galleryid']) )
			exit;
		...
		header("Content-Disposition: attachment; filename=video_tv_slide_export.xml");
	...
	function generate_schema_code($id, $slides, $itemTime) {
		...
		$ret = "\r\n\r\n" . '<div id="video-tv-slide-schema-markup-' . $id . '" class="video-tv-slide-schema-markup" style="display:none;">';
	...
	function generate_body_code($id, $data_attributes, $has_wrapper) {
		...
		if ($item_row != null)
		{
			...
			// if ('F' == 'F')
			// 	$ret .= '<div class="wonderplugin-engine"><a href="http://www.wonderplugin.com/wordpress-gallery/" title="'. get_option('video-tv-slide-engine')  .'">' . get_option('video-tv-slide-engine') . '</a></div>';
			$ret .= '</div>';
			...
			$ret .= '<script>jQuery(document).ready(function(){jQuery(".pgwSlider").pgwSlider({autoSlide:false})});</script>';
	...
	function get_items_from_folder($slide) {
		...
		if (!is_readable($dir) || !file_exists($dir))
		{
			$item = array(
					...
					'image'			=> '',
					'thumbnail'		=> '',
					'title'			=> 'No permissions to browse folders or directory does not exist.',
					'description'	=> '',
					'weblink'		=> '',
					'linktarget'	=> ''
			);
	...
	function get_post_items($options) {
		...
		foreach($posts as $post)
		{
			...
			$excerpt = video_tv_slide_wp_trim_words($excerpt, $options->excerptlength);
	...
	function get_settings() {
		...
		$userrole = get_option( 'video_tv_slide_userrole' );
		if ( $userrole == false )
		{
			update_option( 'video_tv_slide_userrole', 'manage_options' );
			$userrole = 'manage_options';
		}
		
		$keepdata = get_option( 'video_tv_slide_keepdata', 1 );
		$disableupdate = get_option( 'video_tv_slide_disableupdate', 0 );
	...
	function save_settings($options) {
		...
		if (!isset($options) || !isset($options['userrole']))
			...
			$userrole = 'manage_options';
		update_option( 'video_tv_slide_userrole', $userrole );
		
		if (!isset($options) || !isset($options['keepdata']))
			$keepdata = 0;
		else
			$keepdata = 1;
		update_option( 'video_tv_slide_keepdata', $keepdata );
		
		if (!isset($options) || !isset($options['disableupdate']))
			$disableupdate = 0;
		else
			$disableupdate = 1;
		update_option( 'video_tv_slide_disableupdate', $disableupdate );
		
		if (!isset($options) || !isset($options['supportwidget']))
			$supportwidget = 0;
		else
			$supportwidget = 1;
		update_option( 'video_tv_slide_supportwidget', $supportwidget );
		
		if (!isset($options) || !isset($options['addjstofooter']))
			$addjstofooter = 0;
		else
			$addjstofooter = 1;
		update_option( 'video_tv_slide_addjstofooter', $addjstofooter );
		
		if (!isset($options) || !isset($options['jsonstripcslash']))
			$jsonstripcslash = 0;
		else
			$jsonstripcslash = 1;
		update_option( 'video_tv_slide_jsonstripcslash', $jsonstripcslash );
	}
		
	function get_plugin_info() {
		
		$info = get_option('video_tv_slide_information');
		if ($info === false)
			return false;
	...
	function save_plugin_info($info) {
		...
		update_option( 'video_tv_slide_information', serialize($info) );
	}
	...
	function check_license($options) {
		...
		if ( !isset($options) || empty($options['video-tv-slide-key']) )
		{
			return $ret;
		}
		
		$key = sanitize_text_field( $options['video-tv-slide-key'] );
		if ( empty($key) )
			return $ret;
