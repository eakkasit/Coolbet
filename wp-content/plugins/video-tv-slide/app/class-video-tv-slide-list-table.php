...
class Video_TV_Slide_List_Table extends WP_List_Table {
	...
	function get_columns()
	{
		$columns = array(
				...
				'id' => __('ID', 'video_tv_slide'),
				'name' => __('Name', 'video_tv_slide'),
				'shortcode' => __('Shortcode', 'video_tv_slide'),
				'phpcode' => __('PHP code', 'video_tv_slide'),
				'time' => __('Created', 'video_tv_slide')
		);
		return $columns;
	}
	...
	function column_default( $item, $column_name )
	{
		...
		switch( $column_name ) {
			case 'id':
				...
				else
				{
					$actions = array(
							...
							'view' => sprintf('<a href="?page=%s&itemid=%s">View</a>', 'video_tv_slide_show_item', $item['id']),
							'edit' => sprintf('<a href="?page=%s&itemid=%s">Edit</a>', 'video_tv_slide_edit_item', $item['id'])
					);
				}
			...
			case 'shortcode':
				return esc_attr('[video_tv_slide id="' . $item['id'] . '"]');
			case 'phpcode':
				return esc_attr('<?php echo do_shortcode(\'[video_tv_slide id="' . $item['id'] . '"]\'); ?>');
			default:
				return $item[ $column_name ];
		}
	}
	...
	function get_views(){
		...
		// All
		$all_url = admin_url('admin.php?page=video_tv_slide_show_items');
		$class = ($current == 'all' ? ' class="current"' :'');
		$views['all'] = "<a href='" . $all_url . "' " . $class . " >All (" . $this->published . ")</a>";
	
		// Published
		$publish_url = admin_url('admin.php?page=video_tv_slide_show_items&item_status=publish');
		$class = ($current == 'publish' ? ' class="current"' :'');
		$views['publish'] = "<a href='" . $publish_url . "' " . $class . " >Published (" . $this->published . ")</a>";
	
		// Trash
		$trash_url = admin_url('admin.php?page=video_tv_slide_show_items&item_status=trash');
